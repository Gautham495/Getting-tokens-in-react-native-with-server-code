...
export default function Live(props) {
  ...
  const get = async()=>{
    try{
    const token = await axios.get("http://192.168.225.202:8080/access_token?channelName=test")
    alert("Here is the token you can use in this app:",token)
    setPresentToken(token)
    setuserdata([{...userdata,token:presentToken}])
